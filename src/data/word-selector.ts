...
import { sampleOne, type RandomSource } from "./random";
import type { WordGroup } from "./word-database";
import { normalizeSelectedTopics, type WordTopic } from "./word-topics";

export type WordSelectionErrorCode =
  | "NO_TOPICS_SELECTED"
  | "NO_ELIGIBLE_GROUPS"
  | "INSUFFICIENT_WORD_GROUPS";
...
export function filterGroupsByTopics(
  database: readonly WordGroup[],
  selectedTopics: readonly WordTopic[],
): WordGroup[] {
  ...
  return database.filter((group) =>
    group.topics.some((topic) => selected.has(topic)),
  );
}

function hasCommonSelectedTopic(
  groupA: WordGroup,
  groupB: WordGroup,
  selectedTopics: readonly WordTopic[],
): boolean {
  ...
  return groupA.topics.some(
    (topic) => selected.has(topic) && groupB.topics.includes(topic),
  );
}
...
function differentGroupPairs(
  ...
  for (const groupA of groups) {
    ...
    for (const groupB of groups) {
      if (
        groupA.id === groupB.id ||
        groupB.related.length < 1
      ) {
        continue;
      }

      if (
        hasCommonSelectedTopic(
          groupA,
          groupB,
          selectedTopics,
        )
      ) {
        pairs.push([groupA, groupB]);
      }
    }
  }
...
export function selectionAvailability(
  input: Omit<SelectGameWordsInput, "random">,
): WordSelectionErrorCode | null {
  ...
  const eligible = filterGroupsByTopics(
    input.database,
    selectedTopics,
  );

  if (eligible.length === 0) {
    return "NO_ELIGIBLE_GROUPS";
  }

  if (
    input.mode === "similar" &&
    !eligible.some(
      (group) => group.related.length >= input.imposterCount + 1,
    )
  ) {
...
export function selectGameWords(
  ...
  const availability = selectionAvailability({
    ...input,
    selectedTopics,
  });

  if (availability) {
    throw new WordSelectionError(availability);
  }
  ...
  if (input.mode === "similar") {
    const group = sampleOne(
      eligible.filter(
        (candidate) =>
          candidate.related.length >= input.imposterCount + 1,
      ),
      random,
    );

    const civilianWord = sampleOne(group.related, random);

    const imposterWord = sampleOne(
      group.related.filter((word) => word !== civilianWord),
      random,
    );
  ...
  const civilianWord = sampleOne(groupA.related, random);
  const imposterWord = sampleOne(groupB.related, random);
