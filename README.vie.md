...
Repository này bao gồm **hai cấu trúc mã nguồn / kiến trúc triển khai**:
1. **Mô hình Frontend Hiện đại / Offline-first (`src/`, Vite + TypeScript, PWA, Capacitor Android)**:
   - Xây dựng hoàn toàn bằng TypeScript và Vite.
   - Game engine thuần độc lập (`src/core/`), quản lý state có version & migration (`src/storage/`), quản lý bảo mật & quyền riêng tư (`src/security/`), và UI DOM component.
   - Hoạt động offline mượt mà trên Web, PWA hoặc ứng dụng Android native qua Capacitor (`android/`). Được kiểm thử toàn diện bằng Vitest và Playwright.
2. **Mô hình Full-stack Cổ điển (`backend/` & `frontend/`)**:
   - **Backend FastAPI (`backend/`)**: Máy chủ thẩm quyền với đồng bộ WebSocket real-time, công cụ quản lý kho từ CLI (`manage_words.py`), hỗ trợ lưu trữ qua Local CSV, Cloudflare R2 và Cloudflare D1 SQLite.
   - **Frontend Tĩnh (`frontend/`)**: Giao diện client HTML/JS truyền thống kết nối với backend.
